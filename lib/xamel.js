...
NodeSet.prototype.get = function(expr) {
    return (expr === '.') ? this : new NodeSet(this.childs.filter(is.bind(null, expr)));
};

/**
 * Creates node set of nodes meeting the path condition.
 * @param {String} expr XPath-like expression.
 * @return {NodeSet}
 */
...
var is = xamel.is = function(expr, node) {
    return (expr === 'node()' && isNode(node)) ||
        ...
        ((node instanceof Tag) && (expr === '*' || expr === node.name));
};
