...
NodeSet.prototype.get = function(expr) {
    return (expr === '.') ? this : new NodeSet(this.filter(is.bind(null, expr)));
};

/**
 * Creates node set of nodes meeting the condition.
 * @param {String|Array} expr XPath-like conditional expression.
 * @param {Boolean} cont private flag argument
 * @return {NodeSet}
 */
NodeSet.prototype.find = function(expr, cont) {
    var _expr = (typeof expr === 'string') ? expr.split('/') : expr,
        // @fixme now '/node1/node2/…' interpreted as './node1/node2/…'
        currentExpr = _expr.shift() || '.',
        matched = ( ! cont) ? this.get(currentExpr) : this.reduce(function(result, child) {
                if (child instanceof NodeSet) {
                    child.get(currentExpr).forEach(result.append.bind(result));
                }
                return result;
            }, new NodeSet());

    return (_expr.length && matched.length) ? matched.find(_expr, true) : matched;
};
